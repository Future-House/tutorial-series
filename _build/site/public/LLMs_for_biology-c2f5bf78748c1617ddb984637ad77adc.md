...
Most modern natural language models including those in biology—are built using a neural network architecture called the `transformer`.

Before transformers, many sequence models relied on architectures such as recurrent neural networks (RNNs) that processed sequences one element at a time. While effective, these models struggled to capture long-range relationships within sequences.

Transformers address this limitation using a mechanism called attention.
